...
func axPress(_ e: AXUIElement) -> Bool {
    ...
    // Retrying on failure causes a double-click (toggle OFF) for solo/mute/filters/checkboxes.
    // Fire-and-forget: callers that need confirmation verify state separately via AX read.
...
// Click AX button without moving mouse — #1: retry once on transient AX failure
@discardableResult
func axClickDirect(_ e: AXUIElement) -> Bool {
    var r = AXUIElementPerformAction(e, kAXPressAction as CFString)
    if r != .success {
        Thread.sleep(forTimeInterval: 0.20)
        r = AXUIElementPerformAction(e, kAXPressAction as CFString)
    }
    return r == .success
}

// Type text directly into Logic process
// #2/#26: iterate by Character (grapheme cluster) and encode as UTF-16 so emoji and
//         multi-codepoint sequences (e.g. 🎛 = U+1F39B, surrogate pair D83C+DF9B) type correctly
func typeTextToLogic(_ pid: pid_t, _ text: String) {
    for char in text {                          // Character = one grapheme cluster
        let utf16 = Array(String(char).utf16)  // UTF-16 code units (surrogate pairs handled)
        let src = CGEventSource(stateID: .hidSystemState)
...
case "getSelectedTracks":
    // Read AXSelectedChildren from the Mixer's AXLayoutArea — Logic syncs Arrange
    // selection to mixer channel-strips and exposes it directly via this attribute.
    // This is the source of truth: no ordinal math, no name-matching, no assumptions
    // about mixer order vs Arrange order.
    var allWinsRef: CFTypeRef?
    ...
    if windows.isEmpty {
        ...
        windows = (ref as? [AXUIElement]) ?? []
    }
    // Find the largest Mixer layout (standalone Mixer wins over embedded Tracks Mixer)
    var bestLayout: (layout: AXUIElement, count: Int)? = nil
    for win in windows {
        for layout in findAll(win, role: "AXLayoutArea", title: "Mixer") {
    ...
    guard let layout = bestLayout?.layout else {
        jsonOut(["ok": true, "tracks": []]); break
    }
    let items = axKids(layout).filter { axRole($0) == "AXLayoutItem" }
    let selectedRef = axVal(layout, "AXSelectedChildren")
    let selectedItems = (selectedRef as? [AXUIElement]) ?? []
    var result: [[String: Any]] = []
    for sel in selectedItems {
        guard let idx = items.firstIndex(where: { CFEqual($0, sel) }) else { continue }
        let name = (axVal(sel, kAXDescriptionAttribute) as? String) ?? ""
        result.append(["name": name, "index": idx])
    }
...
case "openMixer":
    ...
    // Close Library, Inspector, Browsers, etc. — skip if closePanels already ran (skipPanelClose flag)
    let omSkipPanelClose = args.contains("skipPanelClose")
    if !omSkipPanelClose {
    ...
    // Position mixer at 60% of window height so it's comfortably visible
    ...
    if let omMg = omFindMixerGroup(omWin) {
        ...
        if omDiff > 20 {
            ...
            CGWarpMouseCursorPosition(omDragFrom); usleep(120_000)
            CGEvent(mouseEventSource: nil, mouseType: .leftMouseDown,
...
case "set-locators-by-marker":
    ...
    // Find "Global Tracks" toggle checkbox anywhere in the window (broad search, not fragile path)
    func slbmFindGTCheckbox(_ win: AXUIElement) -> AXUIElement? {
        func search(_ el: AXUIElement, _ d: Int) -> AXUIElement? {
            if d > 14 { return nil }
            if axRole(el) == "AXCheckBox" {
                let desc = (axVal(el, kAXDescriptionAttribute) as? String ?? "").lowercased()
                if desc.contains("global") { return el }
            }
            for k in axKids(el) { if let f = search(k, d+1) { return f } }
            return nil
        }
        return search(win, 0)
    }
    // Find any AXPopUpButton with description containing "Marker" anywhere in the window
    func slbmFindMarkerPopup(_ win: AXUIElement) -> AXUIElement? {
        func search(_ el: AXUIElement, _ d: Int) -> AXUIElement? {
            if d > 14 { return nil }
            if axRole(el) == "AXPopUpButton" {
                let desc = (axVal(el, kAXDescriptionAttribute) as? String ?? "").lowercased()
                if desc.contains("marker") { return el }
            }
            for k in axKids(el) { if let f = search(k, d+1) { return f } }
            return nil
        }
    ...
    if let tw = slbmTracksWin(logic) {
        ...
        if slbmFindMarkerPopup(tw) != nil {
            hadGlobalTracks = true
            fputs("[set-locators-by-marker] Global Tracks already visible with Marker\n", stderr)
        } else {
            // Find GT checkbox with broad search (not fragile hardcoded path)
            let cb = slbmFindGTCheckbox(tw)
            let cbVal = cb.map { axIntValue($0) } ?? -1
            hadGlobalTracks = cbVal == 1
            fputs("[set-locators-by-marker] GT checkbox found=\(cb != nil) val=\(cbVal)\n", stderr)
            if !hadGlobalTracks, let cb = cb {
                axPress(cb); Thread.sleep(forTimeInterval: 0.45)
            } else if !hadGlobalTracks {
                fputs("[set-locators-by-marker] WARNING: GT checkbox not found at all\n", stderr)
            }
            // If Marker track still not visible after showing GT — enable via Track menu
            if slbmFindMarkerPopup(tw) == nil {
                fputs("[set-locators-by-marker] Marker popup missing — enabling via Track menu\n", stderr)
                slbmEnableMarkerTrack(logic); Thread.sleep(forTimeInterval: 0.25)
            }
        }
    } else {
    ...
    // Hide Global Tracks again if we showed them (broad search, same as show)
    if !hadGlobalTracks && setLocatorsOk {
        if let tw = slbmTracksWin(logic), let cb = slbmFindGTCheckbox(tw) {
