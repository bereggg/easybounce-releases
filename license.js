...
async function validateKey(key) {
  ...
  const newFormat = /^EB-[A-F0-9]{16}$/.test(key);
  ...
  try {
    ...
    return { valid: true, email: license.email };

  } catch (e) {
    return { valid: false, reason: 'Could not connect to activation server. Check your internet connection.' };
  }
}

module.exports = { validateKey, getMachineId, getTrialInfo, activateTrial };
